...
iomux_v3_cfg_t const custom1_led_pads[] = {
	IOMUX_PADS(PAD_GPIO_2__GPIO1_IO02 | MUX_PAD_CTRL(NO_PAD_CTRL)), // YEL_LED#
	IOMUX_PADS(PAD_GPIO_9__GPIO1_IO09 | MUX_PAD_CTRL(NO_PAD_CTRL)), // EN_LED2#
	IOMUX_PADS(PAD_EIM_D27__GPIO3_IO27 | MUX_PAD_CTRL(NO_PAD_CTRL)), // EN_LED3#
	IOMUX_PADS(PAD_DISP0_DAT7__GPIO4_IO28 | MUX_PAD_CTRL(NO_PAD_CTRL)), // EN_LED4#
	IOMUX_PADS(PAD_EIM_D23__GPIO3_IO23 | MUX_PAD_CTRL(NO_PAD_CTRL)), // EN_LED5#
	IOMUX_PADS(PAD_DISP0_DAT10__GPIO4_IO31 | MUX_PAD_CTRL(NO_PAD_CTRL)), // EN_LED6#
};

iomux_v3_cfg_t const posttest_pads[] = {
	IOMUX_PADS(PAD_SD4_DAT6__GPIO2_IO14 | MUX_PAD_CTRL(LOOP_PAD_CTRL)), // UART2_CTS
	IOMUX_PADS(PAD_SD4_DAT5__GPIO2_IO13 | MUX_PAD_CTRL(LOOP_PAD_CTRL)), // UART2_RTS
};
...
int emmc_test(void)
{
	printf("Test Disabled : eMMC test passed\n");
	return 0;
}
...
int mem_test(void)
{
	printf("Test Disabled : RAM test passed\n");
	return 0;
}

int usbhub_test(void)
{
	printf("Test Disabled : USB test passed\n");
	return 0;
}
...
int do_post_test(struct cmd_tbl *cmdtp, int flag, int argc, char * const *argv)
{
	int ret = 0;
	int build_variant = 0;
	int opt_r37;
	...
	SETUP_IOMUX_PADS(posttest_pads);
	...
	switch (build_variant) {
		...
		case 15: /* Custom board or failure */
			...
			if(silab_rev() == 1 && fpga_rev() == 6)
			{
				...
				/* Turn on unique LEDs */
				SETUP_IOMUX_PADS(custom1_led_pads);
